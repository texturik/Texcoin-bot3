    ...
    async def process_gamble(self, interaction: discord.Interaction, bet_amount):
        ...
        if random.random() < 0.001:  # 0.1% шанс
            ...
            await interaction.response.edit_message(
                content=f"@everyone 🎉 Поздравляем {interaction.user.mention} с выигрышем ДЖЕКПОТА 2,000,000 {COIN_EMOJI}! 🎉",
                embed=embed, 
                view=None
            )
            return
...
async def tex_help(ctx):
    ...
    embed.add_field(
        name="🎰 КАЗИНО",
        value="```\n"
              "!тексгамба - Открыть казино с разными ставками\n"
              "   • 100 ☢️ - 30% выигрыш (100-200)\n"
              "   • 250 ☢️ - 30% выигрыш (250-500)\n"
              "   • 1,000 ☢️ - 40% выигрыш (1000-1500)\n"
              "   • 10,000 ☢️ - 4 разных исхода\n"
              "   • 100,000 ☢️ - 10 разных исходов\n"
              "   • Джекпот: 2,000,000 ☢️ (0.1% шанс)\n"
              "```",
        inline=False
    )
